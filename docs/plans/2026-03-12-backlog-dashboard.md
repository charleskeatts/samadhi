# Revenue Priority Backlog Dashboard Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Add a `/dashboard/backlog` page — a dark-themed, ARR-ranked revenue priority view with table and kanban toggle, wired to real Supabase data.

**Architecture:** Two new schema columns (`category`, `blocker_score`) added via migration. Seed data updated with realistic values. New page fetches features in one query, derives sources via one batched feedback query (no N+1). Five components on one page: KPI row, controls, table view with expandable drawer, kanban view, bottom bar.

**Tech Stack:** Next.js 14 App Router · TypeScript · Supabase client · Tailwind (minimal) + inline styles for design system colors
...
**Files:**
- Create: `supabase/migrations/002_backlog_columns.sql`
...
```bash
git add supabase/migrations/002_backlog_columns.sql
git commit -m "feat: add category + blocker_score columns to feature_requests"
```
...
**Files:**
- Modify: `types/index.ts` (FeatureRequest interface, around line 58)

**Step 1: Add fields to FeatureRequest**

In `types/index.ts`, update the `FeatureRequest` interface:
...
```bash
cd /Users/charleskeatts/Projects/samadhi && npm run build
```

Expected: build succeeds (may warn about SIGTERM — that's fine, ignore it).

**Step 3: Commit**

```bash
git add types/index.ts
git commit -m "feat: add category + blocker_score to FeatureRequest type"
```
...
**Files:**
- Modify: `lib/supabase/seed-demo.ts` (featureRows array, around line 102)

**Step 1: Add category + blocker_score to all 4 feature rows**

Replace the `featureRows` array in `lib/supabase/seed-demo.ts`:
...
git commit -m "feat: add category + blocker_score to demo seed features"
```

---

### Task 4: Add Backlog nav item

**Files:**
- Modify: `app/(dashboard)/layout.tsx` (navItems array, around line 24)

**Step 1: Insert Backlog between Insights and Roadmap**
...
git commit -m "feat: add Backlog nav item to dashboard sidebar"
```

---

### Task 5: Build the backlog page

**Files:**
- Create: `app/(dashboard)/backlog/page.tsx`

**Step 1: Create the page**

Create `app/(dashboard)/backlog/page.tsx` with this complete implementation:
      ...
      if (allIds.length > 0) {
        ...
        if (fbRows) {
          ...
          for (const feat of feats) {
            const sources = [
              ...new Set(feat.feedback_ids.map((id) => deriveSource(fbMap[id] ?? null))),
            ];
            sm[feat.id] = sources;
          }
          setSourceMap(sm);
...
Expected: clean build. TypeScript should be happy since `category` and `blocker_score` are now in the `FeatureRequest` type.

**Step 3: Commit**

```bash
git add "app/(dashboard)/backlog/page.tsx"
git commit -m "feat: revenue priority backlog page — ARR-ranked table + kanban"
```
...
**Step 1: Kill any stale dev server, start fresh**

```bash
lsof -i :3000 -t | xargs kill 2>/dev/null; sleep 1
```

Then start via `preview_start` (use the MCP tool, not npm run dev).

**Step 2: Navigate to the backlog page**

Go to `http://localhost:3000/dashboard/backlog`

Expected:
- Dark `#0A1628` background
- 4 KPI cards at top (Total ARR at Risk, Critical Blockers, Deals Affected, Filtered ARR)
- Category filter pills: All · Integration · Analytics
- Table view showing 4 features sorted by ARR: Salesforce ($515K) → Bulk Export ($335K) → Slack ($265K) → Custom Reporting ($55K)
- Top 3 ranks highlighted in gold
- Salesforce row has "Deal Blocker" tag (blocker_score: 5)

**Step 3: Click a row to verify drawer**

Click "Salesforce CRM Integration" row.

Expected: inline drawer opens showing Revenue Impact ($515K), Signal Sources (Salesforce Notes), Recommended Action (escalate to CPO).

**Step 4: Click Board toggle**

Expected: 4 kanban columns with ARR totals in column headers.
...
git log --oneline -5  # confirm all 5 task commits are there
